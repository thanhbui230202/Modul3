...
<center>
  ...
  <form action="/furama" method="post">
    <select name="type">
      <option value="employeeName">Tên</option>
  ...
  <form class="form-inline my-2 my-lg-0" method="post" action="/furama?action=search">
...
<%--          <a href="/furama?action=edit&id=${employee.id}">Edit</a>--%>
          <a href="/furama?action=edit&id=${employee.id}" >Edit</a>
          <a onclick="deletebyid(${employee.id})"class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">Delete</a>
        </td>
      </tr>
    </c:forEach>
  </table>
